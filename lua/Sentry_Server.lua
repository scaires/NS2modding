...
function Sentry:OrderChanged()
    ...
    if not self:GetHasOrder() then
        ...
        if orderType == kTechId.Attack and self.desiredMode ~= self.kMode.Reloading then
...
function Sentry:SetFiringSoundState(state)
    ...
    if state ~= self.playingAttackSound then
		//Print("firing sound: state %s self %s",ConditionalValue(state, "true", "false"),ConditionalValue(self.playingAttackSound, "true", "false"))
...
function Sentry:SetMode(mode)
    ...
    if self.mode ~= mode then
        ...
        elseif mode == Sentry.kMode.Reloading then
        
            
			self.reloadingTime = self.timeOfLastUpdate
        end
...
function Sentry:UpdateMode(deltaTime)
    ...
    if self.desiredMode ~= self.mode then
        ...
        elseif self.desiredMode == Sentry.kMode.Reloading then
...
function Sentry:OnAnimationComplete(animName)
    ...
    elseif animName == Sentry.kAttackAnim then
    
        self:SetMode(Sentry.kMode.Attacking)		
		
    elseif animName == Sentry.kAttackEndAnim then

        if self.desiredMode == Sentry.kMode.SettingTarget or self.desiredMode == Sentry.kMode.Reloading then    
            self:SetMode(Sentry.kMode.PoweringDown)
        else
            self:SetMode(Sentry.kMode.Scanning)
        end
    ...
    elseif animName == self:GetPowerDownAnimation() then
        ...
        elseif self.desiredMode == Sentry.kMode.Reloading then    
            self:SetMode(Sentry.kMode.Reloading)
        else
...
function Sentry:UpdateEnergy(timePassed)
end
...
//TCBM acquire fn
function Sentry:TCBMAcquireTarget()
        ...
        if (attackEntValid or attackLocationValid) and (self.timeNextAttack == nil or (currentTime > self.timeNextAttack)) then
            ...
            if mode == Sentry.kMode.Attacking then
                ...
                if energy > 0 then
					...
					self:SetEnergy(energy - 1)
				else
					self:SetFiringSoundState(false)
					Shared.PlayWorldSound(nil, Sentry.kSentryScanSoundName, nil, self:GetModelOrigin())
					
					local potentialArmories = GetEntitiesIsaInRadius("Armory", self:GetTeamNumber(), self:GetOrigin(), Armory.kResupplyUseRange*2)
    
					local foundArmory = nil
					for index, entity in pairs(potentialArmories) do
						if entity:isa("Armory") and entity:GetIsBuilt() and entity:GetIsActive() then
							foundArmory = entity
						end
					end
					if foundArmory ~= nil then
						self:SetDesiredMode(Sentry.kMode.Reloading)
					end
				end
...
function Sentry:UpdateAttack(deltaTime)
    ...
    if self:GetIsFunctioning() then
        ...
        elseif (self:GetSentryMode() == Sentry.kMode.Reloading) then 
			if currentTime > (self.reloadingTime + Sentry.kReloadTime) then 
				self:SetEnergy(self:GetMaxEnergy())
				self:PlaySound(AmmoPack.kPickupSound)
				self:SetMode(Sentry.kMode.PoweringUp)
				self:SetDesiredMode(Sentry.kMode.Scanning)
				self.reloadingTime = currentTime
			end
		else
                ...
                if Server then
					if trace.entity and trace.entity.TakeDamage then
						...
						if(trace.entity:isa("Player")) then
							...
							local impulseVelocity = GetNormalizedVector(direction) * ((50) / trace.entity:GetMass())
...
function Sentry:SetOrder(order, clearExisting, insertFirst)
    ...
    elseif order ~= nil and (order:GetType() == kTechId.Attack) and self.desiredMode ~= self.kMode.Reloading then
        self:SetDesiredMode(Sentry.kMode.Attacking)
    end
